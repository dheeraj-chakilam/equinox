...
module Helpers =
    ...
    type ConsumerCallback = ConsumedTestMessage [] -> Async<unit>
    ...
    let runConsumers log (config : KafkaConsumerConfig) (numConsumers : int) (timeout : TimeSpan option) (handler : ConsumerCallback) = async {
        let mkConsumer (consumerId : int) = async {
            ...
            let consumer = KafkaConsumer.Start log config (fun batch -> handler (Array.map deserialize batch))
        ...
        return! Async.Parallel [for i in 1 .. numConsumers -> mkConsumer i]
    }
...
type T1(testOutputHelper) =
    ...
    let [<FactIfBroker>] ``ConfluentKafka producer-consumer basic roundtrip`` () = async {
        ...
        let consumerList = new List<KafkaConsumer>()

        let stopConsumers() = 
            consumerList
            |> Seq.iter(fun c -> c.Stop())

        let consumerCallback batch = async {
        ...
        let config = KafkaConsumerConfig.Create("eqx-test-client", broker, [topic], groupId)
        let consumers = async {
            let! c = runConsumers log config numConsumers None consumerCallback            
            consumerList.AddRange(c)
            do! consumerList
                |> Seq.map(fun c -> c.AwaitConsumer())
                |> Async.Parallel
                |> Async.Ignore }
